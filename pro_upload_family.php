...
if($_FILES["import_csv_family"]["name"] != '')
{
 $allowed_extension = array('xls', 'csv', 'xlsx');
 $file_array = explode(".", $_FILES["import_csv_family"]["name"]);
 $file_extension = end($file_array);

 if(in_array($file_extension, $allowed_extension))
 {
  $file_name = time() . '.' . $file_extension;
  move_uploaded_file($_FILES['import_csv_family']['tmp_name'], $file_name);
...
$reader->setReadFilter($filterRow);
  ...
  $data = $spreadsheet->getActiveSheet()->toArray();
   $data_control = $spreadsheet->getActiveSheet()->toArray();
  // var_dump($data);
